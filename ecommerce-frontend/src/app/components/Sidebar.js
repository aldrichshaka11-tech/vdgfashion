...
export default function Sidebar({ className = '' }) {
  ...
  return (
    <aside className={`w-80 bg-white border-r border-zinc-200 flex flex-col h-full py-6 px-6 overflow-y-auto no-scrollbar text-black ${className}`}>
      ...
      <div className="flex items-center gap-3 pb-6 border-b border-zinc-150 cursor-pointer" onClick={() => { setSelectedProduct(null); setSelectedCategory('ALL'); router.push('/'); }}>
        <div className="h-11 w-11 bg-linear-to-tr from-pink-500 to-indigo-500 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-sm">
          V
        </div>
